...
Start here for current HQ work. The workspace authority order remains
`AGENTS.md` and `docs/core/SOURCE_OF_TRUTH.md`; this guide is their operational
HQ entrypoint.

| Document | Use it for | Status |
|---|---|---|
| `AI_README.md` | current HQ operating model, evidence rules, safe workflows and release checks | **active operational entrypoint** |
| `README.md` | local setup and repository-oriented utility notes | supporting reference; subordinate to this guide |
| `OPERATING_MODEL.md` | original parallel-validation design | historical migration context; not current authority |
| `MIGRATION_TO_HQ.md` | initial Sheet-to-HQ migration record and gates | historical migration context; cutover superseded |
| `../supabase/README.md` | initial schema migration note | historical setup context; not current ledger authority |

Do not use a historical document to revive a Sheet sync, a pre-cutover ledger
rule, or a prototype-only interpretation of HQ. When these documents conflict,
use `SOURCE_OF_TRUTH.md`, `CURRENT_STATE.md`, then this guide.
...
8. **Do not touch unrelated workspace changes.** Keep every change file-scoped;
   never use deprecated local `.git` artifacts for status, staging, commits or
   bulk operations.
...
`OPERATING_MODEL.md`, `MIGRATION_TO_HQ.md`, and `../supabase/README.md` are
migration-era context. Their status headers make that explicit; they are useful
history, not permission to undo the HQ cutover.
...
| Gmail intake | Turns Vinted transaction mail into evidence, guarded ledger actions or review work | A sale auto-books only on one unique title match to an active listed DEN |
| `scripts/` / tests | Local validation and operational utilities | Use only after checking scope and inputs |

Repository: `PanDory1992/fadewell-hq`. Use the approved remote GitHub release
path when a release is authorized; local `.git` artifacts are deprecated and
must not be used as repository state or a release route.
...
- a `SALE_PENDING` message auto-books only when its title matches exactly one
  `LISTED-BACKLOG` DEN and it contains a positive amount; the match and mail
  remain attached as evidence;
- every other sale message — unmatched, ambiguous, unlisted or missing an
  amount — stays in the review queue until an operator resolves it;
- known non-accounting Vinted mail (for example, a shipping label) is retained
  as an auditable `NOISE / AUTO_DISMISSED` event and never touches the ledger;
...
Never auto-sell an item because a Vinted listing disappears or a mail merely
resembles a sale notification; the unique active-listing match and positive
amount are both required.
...
1. Inspect the affected workspace files directly and preserve unrelated work;
   do not use local Git status because local `.git` artifacts are deprecated.
...
runner belongs to Step 3; until it exists, no change to parser classification or
field-extraction logic is deployable. Evidence-only instrumentation that
preserves those outputs is permitted when it is independently database-verified.
Once introduced, every parser change must run against the complete available
fixture set. A fixture failure, an unrecognized required field, or a newly
observed mail shape blocks deployment until the parser either handles it safely
or routes it to a visible review/exception state. Keep the original source
evidence private and preserve its provenance in HQ; test copies are not a
...
1. Use the approved remote GitHub release route only when a release is
   authorized; do not use local Git artifacts.
2. Restrict the remote change to files belonging to the task and inspect its
   remote diff and checks.
3. Merge only with user authorization or an established explicit release
   instruction.
4. Verify the GitHub Pages deployment and the live behavior at
   `https://hq.fadewell.eu` for UI changes.
5. For a docs-only change, verify the approved remote change; a Pages deploy
   is not a functional requirement unless a web asset changed.
