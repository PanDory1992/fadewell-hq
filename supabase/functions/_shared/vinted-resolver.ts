...
function score(listing:Listing,item:Item){
  const listingText=`${listing.title||''} ${listing.description||''}`,itemText=`${item.name||''} ${item.manual_title||''} ${item.live_title||''} ${item.category||''} ${item.advantage||''}`;
  ...
  const listingSizes=sizes(listingText),itemSizes=sizes(itemText),listingWaists=set([...listingSizes].filter(value=>value.startsWith('w'))),itemWaists=set([...itemSizes].filter(value=>value.startsWith('w'))),listingLengths=set([...listingSizes].filter(value=>value.startsWith('l'))),itemLengths=set([...itemSizes].filter(value=>value.startsWith('l')));
  const conflictingSize=(listingWaists.size>0&&itemWaists.size>0&&overlap(listingWaists,itemWaists).size===0)||(listingLengths.size>0&&itemLengths.size>0&&overlap(listingLengths,itemLengths).size===0);
  if(conflictingSize){points-=60;reasons.push('sprzeczny pełny rozmiar');}
  ...
  const relistStrong=Boolean(item.vinted_item_id)&&completeSize&&shared.size>=4&&alignedRelistAttribute&&!conflictingSize;
  const newListingStrong=!item.vinted_item_id&&completeSize&&sameModels.size>0&&!conflictingSize;
  if(relistStrong)reasons.push('jednoznaczny profil relistu');
  return{item,score:points,reasons,strong:item.vinted_item_id?relistStrong:newListingStrong};
}
