-- Preserve the most recently observed listing photo when a DEN becomes SOLD.
alter table public.hq_ledger_items add column if not exists last_photo_url text;

create or replace function public.apply_hq_ledger_action(p jsonb)
returns bigint language plpgsql security definer set search_path=public as $$
declare event_id bigint; kind text:=p->>'action_type'; item text:=p->>'item_id'; event_date date:=nullif(p->>'occurred_on','')::date; event_source text:=coalesce(nullif(p->>'source',''),'MANUAL'); action_amount numeric:=nullif(p->>'amount','')::numeric;
begin
  ...
  if kind='PURCHASE' then
    ...
    insert into hq_ledger_items(item_id,name,sourcing_type,flip_tier,purchase_cost,delivery_cost,total_capital,ledger_status,purchased_on,source_row,migration_state,version) values(item,p->>'name',p->>'sourcing_type',p->>'flip_tier',nullif(p->>'purchase_cost','')::numeric,nullif(p->>'delivery_cost','')::numeric,nullif(p->>'total_capital','')::numeric,'UNLISTED-BACKLOG',event_date,'{}'::jsonb,'CANONICAL',1);
  ...
  elsif nullif(btrim(coalesce(p->>'note','')),'') is null then raise exception 'ADJUSTMENT requires a written reason; it does not rewrite prior events'; end if;
