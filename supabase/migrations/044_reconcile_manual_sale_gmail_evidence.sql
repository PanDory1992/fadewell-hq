-- A manual sale can be recorded before delayed Gmail intake resumes. Link the
-- mail as evidence only when one existing manual sale is uniquely identified;
-- never create, alter, or infer a ledger sale here.
...
begin
  ...
  into candidate_count,matched_ledger_event_id,matched_item_id,matched_amount
  from (
  ...
  amount_delta:=round(event_row.amount-matched_amount,2);
  update public.hq_external_events
  set state='MANUAL_RESOLVED',
      matched_item_id=matched_item_id,
      ledger_event_id=matched_ledger_event_id,
  ...
  return jsonb_build_object('reconciled',true,'state','MANUAL_RESOLVED','ledger_event_id',matched_ledger_event_id,'item_id',matched_item_id,'amount_delta',amount_delta);
end $$;
