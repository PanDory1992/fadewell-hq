-- Gmail sale notifications are evidence only. A SALE_PENDING event must stay
-- in review until an owner records the confirmed sale through migration 023's
-- resolve_hq_gmail_review_owner path.
...
begin
  ...
  if kind='PURCHASE_BUNDLE' and (p->>'state')='AUTO_APPLIED' then
    ...
    raise exception 'Only a complete purchase receipt or complete bundle receipt may auto-apply';
  end if;
  if was_review then
