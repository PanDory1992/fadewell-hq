-- PostgreSQL uses \m / \M for word boundaries; \b is a backspace escape.
-- Rebuild the title function with literal wash detection that also catches “Mid Blue”.
...
begin
  ...
  if lower_title ~ '\mmade in usa\M|\musa\M' then inferred:=inferred||jsonb_build_object('origin','USA');
  elsif lower_title ~ '\mmade in japan\M|\mjapan-made\M|\mjapan\M' then inferred:=inferred||jsonb_build_object('origin','Japan');
  elsif lower_title ~ '\mmade in mexico\M|\mmexico\M' then inferred:=inferred||jsonb_build_object('origin','Mexico');
  elsif lower_title ~ '\mmade in poland\M|\mpoland\M' then inferred:=inferred||jsonb_build_object('origin','Poland'); end if;
  ...
  if lower_title ~ '\m(mid|medium)[ -]blue\M' then inferred:=inferred||jsonb_build_object('wash','mid blue');
  elsif lower_title ~ '\mdark (blue|indigo)\M' then inferred:=inferred||jsonb_build_object('wash','dark indigo');
  elsif lower_title ~ '\mblack\M' then inferred:=inferred||jsonb_build_object('wash','black'); end if;
  if inferred='{}'::jsonb then return new; end if;
  new.item_dna:=jsonb_build_object('schema_version',1,'facts',inferred||old_facts,'evidence',coalesce(new.item_dna->'evidence','{}'::jsonb)||jsonb_build_object('auto_from_listing_title',title,'source','VINTED_TITLE'),'updated_at',now(),'updated_by','SYSTEM');
  new.item_dna_updated_at:=now(); return new;
end $$;
