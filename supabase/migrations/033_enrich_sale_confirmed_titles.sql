-- Completion mail contains a verified Vinted title. Permit a later parser fix
-- to enrich an already auto-recorded completion event without a ledger change.
...
declare
  ...
  old_state text; old_ledger_id bigint; old_item text; old_title text; was_review boolean := false;
...
begin
  ...
  select state,ledger_event_id,matched_item_id,item_title into old_state,old_ledger_id,old_item,old_title
  ...
  if found then
    if old_state in ('AUTO_APPLIED','AUTO_DISMISSED','MANUAL_RESOLVED')
      ...
      if old_state='AUTO_APPLIED' and kind='SALE_CONFIRMED'
         and coalesce(nullif(btrim(old_title),''),'')=''
         and coalesce(nullif(btrim(p->>'item_title'),''),'')<>'' then
        update hq_external_events set
          item_title=nullif(btrim(p->>'item_title'),''),
          vinted_transaction_id=coalesce(nullif(p->>'vinted_transaction_id',''),vinted_transaction_id),
          evidence=coalesce(evidence,'{}'::jsonb)||jsonb_build_object('title_enriched_at',now())
        where source='GMAIL_VINTED' and source_event_id=source_id;
        return jsonb_build_object('duplicate',false,'state','AUTO_APPLIED','ledger_event_id',old_ledger_id,'item_id',old_item,'enriched',true);
      end if;
