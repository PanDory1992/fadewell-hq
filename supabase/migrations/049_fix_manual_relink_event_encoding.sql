-- Repair the one affected annotation and make relink event text encoding-safe.
-- This is text-only: DEN identity, amount, date and source remain unchanged.
...
begin
  ...
  values (
    ...
    event_date,
    action_amount,
    format(U&'Operations: r\0119cznie potwierdzone od\015Bwie\017Cenie oferty %s \2192 %s.', previous_vinted_id, requested_vinted_id),
    'MANUAL',
...
insert into public.hq_ledger_events(item_id, event_type, occurred_on, amount, detail, source, external_key)
select 'DEN-189', 'ADJUSTMENT', current_date, null, 'Technical text correction for ledger event 333; no business values changed.', 'SYSTEM', 'system:manual-relink-text-correction:333'
where not exists (
  select 1 from public.hq_ledger_events where external_key = 'system:manual-relink-text-correction:333'
);
