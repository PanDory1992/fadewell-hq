-- A relist changes a Vinted listing ID, but it must never silently overwrite a
-- still-live listing identity. The explicit relist flag keeps the event trail.
...
begin
  ...
  if kind='PURCHASE' then
    ...
    if requested_vinted_id is not null and current_vinted_id is not null and requested_vinted_id<>current_vinted_id then
      ...
      if exists (
        select 1 from hq_listing_snapshots s
        where s.source='github_actions_vinted'
          and s.captured_at=(select max(captured_at) from hq_listing_snapshots where source='github_actions_vinted')
          and s.vinted_item_id=current_vinted_id
      ) then raise exception 'Cannot replace a Vinted listing still present in the latest complete snapshot'; end if;
    end if;
  ...
  insert into hq_ledger_events(item_id,event_type,occurred_on,amount,detail,source,external_key) values(item,kind,event_date,action_amount,p->>'note',event_source,p->>'external_key') returning id into event_id;
  return event_id;
end $$;
