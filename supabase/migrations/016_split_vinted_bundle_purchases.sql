-- Vinted sends one paid total for a bundle. Split it exactly in grosze: every
-- item gets an equal share and any remaining grosz goes to the first items.
-- The Google Sheet remains untouched.

alter table public.hq_external_events drop constraint if exists hq_external_events_event_type_check;
alter table public.hq_external_events add constraint hq_external_events_event_type_check check (event_type in ('UNCLASSIFIED','PURCHASE_CONFIRMED','PURCHASE_BUNDLE','SALE_PENDING','SALE_CONFIRMED'));
...
begin
  ...
  select state,ledger_event_id,matched_item_id into old_state,old_ledger_id,old_item from hq_external_events where source='GMAIL_VINTED' and source_event_id=source_id;
  if found then
    if old_state='AUTO_APPLIED' or (p->>'state')<>'AUTO_APPLIED' then return jsonb_build_object('duplicate',true,'state',old_state,'ledger_event_id',old_ledger_id,'item_id',old_item); end if;
    was_review := true;
  end if;
  ...
  elsif (p->>'state')='AUTO_APPLIED' then raise exception 'Only a complete purchase receipt, complete bundle receipt or unambiguous pending sale may auto-apply'; end if;
  if was_review then
...
-- Correct the first real bundle, received before bundle recognition existed.
do $$
begin
  if exists (select 1 from hq_external_events where source='GMAIL_VINTED' and source_event_id='19f5d480c5e5dd66') and exists (select 1 from hq_ledger_items where item_id='DEN-246') then
    update hq_ledger_items set purchase_cost=26.38,total_capital=26.38,version=version+1 where item_id='DEN-246' and total_capital=52.75;
    if not exists (select 1 from hq_ledger_events where external_key='gmail-19f5d480c5e5dd66-bundle-correction') then
      insert into hq_ledger_events(item_id,event_type,occurred_on,detail,source,external_key) values('DEN-246','ADJUSTMENT','2026-07-13','Bundle correction: original 52.75 PLN receipt split equally into DEN-246 (26.38 PLN) and DEN-247 (26.37 PLN).','SYSTEM','gmail-19f5d480c5e5dd66-bundle-correction');
    end if;
    if not exists (select 1 from hq_ledger_items where item_id='DEN-247') then
      perform apply_hq_ledger_action(jsonb_build_object('action_type','PURCHASE','item_id','DEN-247','occurred_on','2026-07-13','name','Levis','sourcing_type','Vinted purchase bundle','purchase_cost',26.37,'delivery_cost',0,'total_capital',26.37,'note','Vinted Gmail bundle receipt 19f5d480c5e5dd66 · part 2/2 · equal split of paid total','source','VINTED','external_key','gmail-19f5d480c5e5dd66-bundle-2'));
    end if;
    update hq_external_events set event_type='PURCHASE_BUNDLE',item_title='Dżinsy L36 · Levis',matched_item_id='DEN-246',evidence=coalesce(evidence,'{}'::jsonb)||jsonb_build_object('bundle_item_count',2,'bundle_item_titles',jsonb_build_array('Dżinsy L36','Levis'),'bundle_item_ids',jsonb_build_array('DEN-246','DEN-247'),'bundle_shares',jsonb_build_array(26.38,26.37),'bundle_corrected',true) where source='GMAIL_VINTED' and source_event_id='19f5d480c5e5dd66';
  end if;
end $$;
