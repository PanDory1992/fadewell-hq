-- Make storefront freshness and storefront completeness separate, visible signals.
-- Incomplete rows remain unpublished; this migration only adds owner-facing
-- classification and review visibility.
...
begin
  ...
  select
    ...
    count(*) filter (
      where available and not sold and publication_status not in ('PUBLISHED','OUT_OF_SCOPE_CATEGORY')
    )::bigint,
...
begin
  ...
  select
    ...
    (c.available and not c.sold and c.derived_publication_status not in ('PUBLISHED','OUT_OF_SCOPE_CATEGORY')),
