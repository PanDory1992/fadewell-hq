-- Vinted uses curly apostrophes (Levi’s); stored titles may omit them (Levis).
-- Apostrophes are formatting inside one word, so remove only apostrophes before
-- the exact normalized-title comparison. No token, date, or amount rule changes.
...
begin
  ...
  from (
    ...
    where le.event_type='SALE'
      ...
      and lower(regexp_replace(replace(replace(coalesce(item.live_title,item.name,''),'’',''),'''',''),'[^[:alnum:]]+',' ','g'))
          = lower(regexp_replace(replace(replace(event_row.item_title,'’',''),'''',''),'[^[:alnum:]]+',' ','g'))
  ) candidate;
