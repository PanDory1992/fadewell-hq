-- Gmail intake may auto-book only a uniquely matched active listing. Known
-- Vinted operational mail is retained as evidence but kept out of human review.

alter table public.hq_external_events drop constraint if exists hq_external_events_event_type_check;
alter table public.hq_external_events add constraint hq_external_events_event_type_check
  check (event_type in ('UNCLASSIFIED','PURCHASE_CONFIRMED','PURCHASE_BUNDLE','SALE_PENDING','SALE_CONFIRMED','NOISE'));

alter table public.hq_external_events drop constraint if exists hq_external_events_state_check;
alter table public.hq_external_events add constraint hq_external_events_state_check
  check (state in ('NEEDS_REVIEW','AUTO_APPLIED','AUTO_DISMISSED','MANUAL_RESOLVED'));
...
begin
  ...
  if kind='PURCHASE_BUNDLE' and (p->>'state')='AUTO_APPLIED' then
    ...
    raise exception 'Only a complete purchase receipt, complete bundle receipt or uniquely matched active listed sale may auto-apply';
