...
const tooEarlyAbove=recommendation(live,model,{latest:{captured_at:'2026-07-12'},snapshotAge:0,price:190,daysLive:18,likes:0,likesDelta:0},{});assert.equal(tooEarlyAbove.action,'OBSERVE');assert.match(tooEarlyAbove.reason,/powyÅ¼ej estymowanego zakresu/);
const underpriced=recommendation(live,model,{latest:{captured_at:'2026-07-12'},snapshotAge:0,price:100,daysLive:30,likes:4,likesDelta:3,historyDays:7},{});assert.equal(underpriced.action,'RAISE');assert.ok(underpriced.nextPrice>100);assert.match(underpriced.reason,/poniÅ¼ej estymowanego zakresu/);console.log('Pricing cockpit tests passed');
