...
const ledgerLabel=type=>({PURCHASE:'Zakup',LISTED:'Wystawiono',SALE:'Sprzedano',TITLE_CHANGE:'Zmieniono tytuł',ADJUSTMENT:'Korekta'}[type]||type||'Zdarzenie Ledgeru');
...
function snapshotEntries(snapshots,item){
  ...
  for(const row of rows.slice(1)){const changes=[];if(present(row.price_pln)&&Number(row.price_pln)!==Number(previous.price_pln))changes.push(`cena zauważona: ${Number(previous.price_pln)} → ${Number(row.price_pln)} PLN`);if(present(row.favourites)&&Number(row.favourites)!==Number(previous.favourites))changes.push(`likes: ${Number(previous.favourites||0)} → ${Number(row.favourites)}`);if(present(row.views)&&Number(row.views)!==Number(previous.views))changes.push(`wyświetlenia: ${Number(previous.views||0)} → ${Number(row.views)}`);if(present(row.visible)&&Boolean(row.visible)!==Boolean(previous.visible))changes.push(row.visible?'oferta znów widoczna':'oferta niewidoczna w tym odczycie');if(changes.length)output.push({id:`snapshot:${row.vinted_item_id}:${row.captured_at}`,at:when(row),precision:'time',type:'observation',title:'Zmiana zaobserwowana na Vinted',summary:changes.join(' · '),source:sourceFor(row),evidence:[evidence(`Vinted #${row.vinted_item_id}`,link)]});previous=row;}
  return output;
}

export function buildTimeline({item,ledgerEvents=[],snapshots=[],externalEvents=[]}){
  const externalsByLedger=new Map();externalEvents.filter(event=>event.ledger_event_id).forEach(event=>{const key=String(event.ledger_event_id),list=externalsByLedger.get(key)||[];list.push(event);externalsByLedger.set(key,list);});
